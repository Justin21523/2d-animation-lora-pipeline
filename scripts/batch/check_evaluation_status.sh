...
ls -lht /mnt/c/ai_models/diffusion/lora/sdxl/coco/miguel_identity/*.safetensors 2>/dev/null | head -1 | \
    awk '{print "  " $9 " (" $5 ") - " $6 " " $7 " " $8}'
echo ""

# Check evaluation outputs
echo "🎨 Evaluation Results:"
eval_dirs=$(find /mnt/c/ai_models/diffusion/lora/sdxl/coco/miguel_identity -maxdepth 1 -type d -name "eval_*" 2>/dev/null | wc -l)
if [ $eval_dirs -gt 0 ]; then
    echo "  ✅ $eval_dirs checkpoint(s) evaluated"
    for eval_dir in $(find /mnt/c/ai_models/diffusion/lora/sdxl/coco/miguel_identity -maxdepth 1 -type d -name "eval_*" 2>/dev/null | sort); do
