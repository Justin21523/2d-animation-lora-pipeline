...
checkpoint_count=0
for char_dir in /mnt/c/ai_models/diffusion/lora/sdxl/*/*/; do
...
log_files=$(find /mnt/c/ai_models/diffusion/lora/sdxl -name "events.out.tfevents*" -mmin -30 2>/dev/null)
if [ -z "$log_files" ]; then
...
echo "🔍 啟動 TensorBoard:   conda run -n kohya_ss tensorboard --logdir /mnt/c/ai_models/diffusion/lora/sdxl/orion/orion_identity/logs --port 6006 --bind_all"
