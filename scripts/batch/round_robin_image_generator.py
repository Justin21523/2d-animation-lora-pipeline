...
Each round generates 1 image per combination (42 images per round).
This allows seeing results from all characters quickly rather than waiting
for one character to complete entirely.

Round structure:
- 14 characters × 3 lora_types = 42 combinations per round
- 100 prompts × 10 images/prompt = 1000 rounds total
- 42 × 1000 = 42,000 images total
...
class RoundRobinGenerator:
    ...
    def __init__(self, config_path: str, resume: bool = True):
        ...
        self.checkpoint_file = self.checkpoint_dir / "round_robin_progress.json"
        ...
        # Calculate totals
        self.num_combinations = len(self.characters) * len(self.lora_types)  # 42
        self.total_rounds = self.num_prompts * self.num_images_per_prompt  # 1000
        self.total_images = self.num_combinations * self.total_rounds  # 42000
    ...
    def _get_output_path(self, character: str, lora_type: str, prompt_idx: int, image_idx: int) -> Path:
        ...
        return output_dir / f"prompt_{prompt_idx:04d}_img_{image_idx:02d}.png"
    ...
    def _get_caption_path(self, character: str, lora_type: str, prompt_idx: int, image_idx: int) -> Path:
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        return output_dir / f"prompt_{prompt_idx:04d}_img_{image_idx:02d}.txt"
    ...
    def generate_round(self, round_num: int) -> int:
        ...
        print(f"\n{'='*70}")
        print(f"🔄 ROUND {round_num + 1}/{self.total_rounds} | Prompt {prompt_idx + 1}/100, Image {image_idx + 1}/10")
        print(f"{'='*70}")
        ...
        for char_idx, character in enumerate(self.characters):
            ...
            for type_idx, lora_type in enumerate(self.lora_types):
                ...
                if output_path.exists():
                    print(f"  [{combo_num}/42] {combo_key}: Already exists, skipping")
                    continue
                ...
                if prompt_idx >= len(prompts):
                    print(f"  [{combo_num}/42] {combo_key}: No prompt at index {prompt_idx}")
                    continue

                prompt = prompts[prompt_idx]['prompt']

                # Generate image
                print(f"  [{combo_num}/42] {combo_key}: Generating...", end=" ", flush=True)
    ...
    def run(self):
        ...
        start_round = self.progress['current_round']
