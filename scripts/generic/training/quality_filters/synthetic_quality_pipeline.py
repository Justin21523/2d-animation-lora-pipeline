...
def _filter_image_worker(args):
    """Worker function for parallel 3-stage image filtering (blur, size, dedup)."""
    ...
    dedup_filter = PerceptualHashDeduplicator(config={
    ...
    try:
        ...
        if not passed_size:
            return ('C', size_reason, blur_variance, img_path)

        # Stage 3: Dedup
        passed_dedup, dedup_reason = dedup_filter.filter_single(img)
        if not passed_dedup:
            return ('C', dedup_reason, blur_variance, img_path)

        # All 3 stages passed - determine tier based on blur score
        tier = 'A' if blur_variance > blur_premium_threshold else 'B'
        return (tier, None, blur_variance, img_path)

    except Exception as e:
        logger.error(f"Error filtering {img_path}: {e}")
        return ('C', f"Error: {str(e)}", None, img_path)
...
class SyntheticQualityPipeline:
    ...
    def process_character_type(
        ...
        results = []
        for tier, reason, blur_score, img_path in raw_results:
            results.append({
                'image': img_path.name,
                'tier': tier,
                'reason': reason,
